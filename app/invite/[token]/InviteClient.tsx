...
.nrinv .env{ position:relative; width:min(720px,92vw); height:calc(min(720px,92vw)*0.66); cursor:pointer; border-radius:14px; overflow:hidden; box-shadow:0 40px 72px -20px rgba(8,13,32,.62), 0 12px 26px rgba(8,13,32,.28); animation:envIn .8s var(--ease) both; }
...
.nrinv .env-seal{ position:absolute; top:calc(52% - 49px); left:50%; transform:translateX(-50%); width:98px; height:98px; background:url("/wax-seal.png") center/contain no-repeat; z-index:6; filter:drop-shadow(0 4px 7px rgba(13,22,46,.4)); transition:opacity .3s ease, transform .5s var(--ease); }
